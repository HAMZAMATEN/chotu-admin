...
class CategoriesProvider with ChangeNotifier {
  ...
  Future<void> addCategory(String categoryName) async {
    try {
      ...
      AppFunctions.showToastMessage(message: "Exception while adding category: $e");
      ShowToastDialog.closeLoader();
    }
  }
  ...
  Future<void> updateCategoryStatus(CategoryModel category) async{
    try{
      http.Response response = await apiServicesProvider.getRequestResponse(APIConstants.updateCategoryStatus+'${category.id}');
      ...
      if(response.statusCode == 200){
        ...
        int index = allCategoriesList!.indexWhere((store) => store.id == category.id);

        if (index != -1) {
    allCategoriesList![index] = tempModel; // Update the store in the list
        }
        AppFunctions.showToastMessage(message: "Category Status Updated Successfully");
      }
      ShowToastDialog.closeLoader();
      notifyListeners();
    }catch(e){
      AppFunctions.showToastMessage(message: "Exception while updateCategoryStatus: $e");
      ShowToastDialog.closeLoader();
    }
  }
  Future<void> deleteCategory(CategoryModel category) async {
    try {
      ...
      final response = await apiServicesProvider.getRequestResponse(
          '${APIConstants.deleteCategory}${category.id}'
      );
      ...
      AppFunctions.showToastMessage(message: "Exception while deleting category: $e");
      ShowToastDialog.closeLoader();
    }
  }
