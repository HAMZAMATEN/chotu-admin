...
class StoreProductProvider extends ChangeNotifier {
  ...
  Future<void> updateProductStatus({required ProductModel product}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("Error while updating product status: ${response.statusCode}");
      }
  ...
  Future<void> pickAndUploadExcel(BuildContext context,StoreModel store) async {
    try {
      ...
      /// These functions pick files from excel and get images bytes and their names
      /// After picking files, they are reordered, accordingly to match order of products
      // Step 1: Decode XLSX (ZIP)
      final archive = await ZipDecoder().decodeBytes(bytes);

      List<Uint8List> imageBytesList = [];
      List<String> imageNames= [];
      // Step 2: Find images in xl/media/
      for (final file in archive) {
        if (file.isFile && file.name.startsWith('xl/media/')) {
          ...
          // You can now upload this image or store it in your map
        }
      }
      ...
      // Sort by file name naturally (e.g., image1 < image2 < image10)
      pairedList.sort((a, b) {
        // Optional: natural numeric sorting
      ...
      // Clear and repopulate lists with sorted order
      imageNames
        ..clear()
      ...
      /// These functions, reads row one by one, and upload the product data along with image bytes
      // Step 2: Decode Excel
      ...
      int total = sheet.maxRows - 1; // Assuming first row is header
      int successCount = 0;
      int failCount = 0;

      EasyLoading.show(status: "Starting upload...");

      // Step 3: Iterate rows (skip header)
      for (int i = 1; i < sheet.rows.length; i++) {
        try {
          ...
          if (name.isEmpty || imageBytesList[i-1].isEmpty) {
            debugPrint("Skipping row $i due to missing data");
            continue;
          }
          String fileName = imageNames[i-1];
          ...
          // Step 6: Set productImageMap globally (used in addProductToDataBase)
          Map<String,dynamic> productMap = {
            'image': imageBytesList[i-1],
            'fileName': fileName,
          };

          // Step 7: Call your existing upload API
          bool productAdded = await addExcelProductToDataBase(body, context, productMap);
      ...
      EasyLoading.dismiss();
      EasyLoading.showToast("Upload complete. Success: $successCount, Failed: $failCount");
  ...
  Future<bool> addExcelProductToDataBase(
      Map<String, dynamic> body, BuildContext context,Map<String,dynamic> productMap) async {
    try {
      final uri =
      Uri.parse(APIConstants.addProduct); // 🔁 Replace with your API
      final request = await http.MultipartRequest('POST', uri);
      ...
      if (response.statusCode == 201) {
        ...
        debugPrint('Product Addition failed: ${await response.stream.bytesToString()}');
        return false;
      }
    } catch (e) {
      debugPrint("EXCEPTION WHILE ADDING Product TO DB ${e}");
